...
const ModifierBeneficiaire = () => {
  ...
  useEffect(() => {
    const fetchBeneficiaire = async () => {
      try {
        const [beneficiaireResponse, centreCoutResponse, directionResponse] = await Promise.all([
  ...
  const handleNomChange = (event: { target: { value: React.SetStateAction<string>; }; }) => {
    setNom(event.target.value);
  };

  const handlePrenomChange = (event: { target: { value: React.SetStateAction<string>; }; }) => {
